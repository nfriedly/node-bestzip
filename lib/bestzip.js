...
// isn't installed. The resolved path — never the bare `zip` name — is used to
// spawn the command, because the child process chdirs into the (possibly
// untrusted) archive directory before execvp. With a bare name, execvp would
// re-resolve `zip` from inside that directory when PATH contains a relative or
// empty element, letting attacker-controlled contents of the directory run as
// the build user (CWE-426/CWE-427).
//
// Resolution deliberately ignores two classes of PATH entries:
//
// - Relative or empty entries, because they are resolved against
//   process.cwd() — the documented `cd build/ && bestzip ../build.zip *`
//   recipe (and the `cwd` option) sets that to the attacker-controlled archive
//   directory itself.
//
// - Entries inside a node_modules tree, because those hold dependency-provided
//   tooling rather than a system binary. npm prepends `<project>/node_modules/
//   .bin` to PATH for every npm script, and any installed dependency can claim
//   the `zip` bin name through its `bin` field. npm creates that shim during
//   install, so `--ignore-scripts` doesn't prevent it; with the shim first in
//   PATH, `which` would hand it back and bestzip would run it as the build
//   user. The native fast path exists to use the OS/system zip, so when the
//   only `zip` on PATH is dependency-supplied, bestzip falls back to its own
//   node implementation (nodeZip) instead.
function isInsideNodeModules(entry) {
  return entry
    .split(path.sep)
...
function getNativeZipCommand() {
  const absolutePath = (process.env.PATH || "")
    ...
    .filter(
      (entry) => entry && path.isAbsolute(entry) && !isInsideNodeModules(entry)
    )
    .join(path.delimiter);
  if (!absolutePath) {
    // Don't probe with an empty `path`: `which` treats "" as a single implicit
    // current-directory entry, which would re-open the cwd-relative resolution
    // this function exists to avoid.
    return null;
  }
  return which.sync("zip", { path: absolutePath, nothrow: true }) || null;
}
