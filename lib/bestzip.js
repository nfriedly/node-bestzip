...
// Whether bestzip's advisory warnings should be printed. Warnings are emitted
// unless explicitly disabled: `warn: false` (the `--no-warn` CLI flag) silences
// them while keeping progress output, and `quiet` silences both. An explicit
// `warn: true` (the `--warn` flag) overrides quiet for warnings only — so
// `--quiet --warn` and `--warn --quiet` behave identically: "run quietly, but
// warn me" (progress suppressed, warnings shown).
...
// null when there was nothing to warn about (or when warnings are suppressed).
// A given path is warned at most once per process so repeated resolution probes
// don't spam the build log. Suppressed probes (`quiet` or `warn: false`) do not
// mark the path as warned, so a later probe with warnings on still reports it.
