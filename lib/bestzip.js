...
const nodeZip = async (options) => {
  ...
  // Close the destination handle exactly once, on every outcome, so the file
  // never stays open and blocks a later rmdir of the enclosing directory.
  ...
  const closeOutput = async () => {
    ...
    try {
      ...
      // Best-effort close; reporting the original error takes precedence.
    }
  };

  output.on("close", () => {
    closeOutput().then(resolvePromise, rejectPromise);
  });
  output.on("error", (err) => {
    closeOutput().catch(() => {});
    rejectPromise(err);
