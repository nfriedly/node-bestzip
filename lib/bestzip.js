...
const nativeZip = async (options) => {
  ...
  // Remove any pre-existing archive at the destination before spawning. The
  // post-exit existence check below must prove a *fresh* archive was written;
  // without this, a stale archive from a previous build would satisfy it even
  // when a broken or compromised `zip` exits 0 without writing anything (and a
  // real `zip` would also update an existing archive rather than rebuild it,
  // keeping stale entries alive).
  try {
    await fsp.unlink(destination);
  } catch (err) {
    if (err.code !== "ENOENT") {
      throw new Error(
        `Unable to remove the existing archive at '${destination}' before zipping: ${err.message}`
      );
    }
  }
