...
//
// Only absolute PATH entries are consulted so that resolution is immune to the
// process cwd: an empty or relative PATH element is resolved against
// process.cwd(), which the documented `cd build/ && bestzip ../build.zip *`
// recipe (and the `cwd` option) sets to the attacker-controlled archive
// directory itself.
