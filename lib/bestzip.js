...
function normalizeArchiveName(entryPath) {
...
const nodeZip = async (options) => {
  ...
  async function addSource(source) {
    ...
    if (stats.isDirectory()) {
      ...
      const normalizedDestPath = normalizeArchiveName(destPath).replace(
        /\/?$/,
        "/"
      );
      ...
      if (files.length === 0) {
        archive.append("", {
          name: normalizedDestPath,
        });
      }

      for (const f of files) {
        const subPath = normalizeArchiveName(path.relative(fullPath, f));
        const fileStats = fs.statSync(f);
        if (fileStats.isDirectory()) {
          archive.append("", {
            name: normalizedDestPath + subPath.replace(/\/?$/, "/"),
          });
        } else {
          archive.file(f, {
            name: normalizedDestPath + subPath,
          });
        }
      }
