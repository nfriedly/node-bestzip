...
// `zip` planted there or in the dependency tree (CWE-426/CWE-427). Only two
// sources are ever checked — an explicit user location (`zipPath` option or
// `BESTZIP_ZIP_PATH`), then the allowlisted system directories above.
...
// Scans PATH read-only to find a `zip` bestzip declined to run, so the user
// can be told which one and how to opt into it (CWE-426/CWE-427 means PATH is
// never used to decide what runs). Returns its path, or null when PATH offers
// nothing.
...
// Whether advisory warnings should be printed. `warn: false` silences them
// while keeping progress output; `quiet` silences both; an explicit
// `warn: true` overrides quiet for warnings only ("run quietly, but warn me").
function warningsEnabled(options) {
  return options.warn !== false && (options.warn === true || !options.quiet);
}

const warnedUntrustedZipPaths = new Set();

// Logs a warning naming a `zip` bestzip declined via PATH and how to use it
// deliberately, and returns the refused path (or null when there's nothing to
// warn about). Warns at most once per path; suppressed probes (`quiet` or
// `warn: false`) don't mark the path, so a later warnings-on probe still
// reports it.
function maybeWarnAboutRefusedZip(options = {}) {
  const { pathEnv = process.env.PATH, viaCli } = options;
  if (!warningsEnabled(options)) {
    return null;
  }
  ...
  const optIn = viaCli
    ...
    : `set zipPath: '${refused}' in code or BESTZIP_ZIP_PATH='${refused}' ` +
      "in the environment";
  const suppress = viaCli
    ? "pass --no-warn to silence just the warnings, or --quiet to silence all non-error output"
    : "set warn: false to silence just the warnings, or quiet: true to silence all non-error output";
  console.warn(
    `Warning: bestzip found a \`zip\` command at '${refused}' but it is not ` +
      ...
      `To suppress this warning, ${suppress}.\n` +
...
const nativeZip = async (options) => {
  ...
  // scan only exists to feed the default symlink warning, so it is skipped
  // whenever warnings are disabled — quiet (or warn: false) skips it
  // altogether, but an explicit warn: true re-enables it even under quiet
  // (saving a full tree walk on large builds when nothing will be reported).
  const warnScan = (async () => {
    if (options.followSymLinks !== undefined || !warningsEnabled(options)) {
      return [];
    }
    try {
  ...
  // Record the destination's state before spawning so a 0 exit can be verified
  // to have actually (re)written the archive. Checking only that the file
  // *exists* afterwards would let a stale archive from a previous build pass
  // even when a broken or compromised `zip` exits 0 without writing anything,
  // so the mtime must have advanced past this pre-spawn snapshot (or the file
  // must newly appear). The destination itself is left untouched — nothing is
  // deleted, only compared.
  let preStat = null;
  try {
    preStat = await fsp.stat(destination);
  } catch (_err) {
    // Either the file didn't exist (err.code === "ENOENT"), or we can't check it
    // for some other reason. Don't worry about it, any real problem will surface
  ...
  const zipPromise = new Promise((resolve, reject) => {
    ...
    zipProcess.on("close", async (exitCode) => {
      if (exitCode === 0) {
        try {
          const postStat = await fsp.stat(destination);
          ...
          // missing artifact). An unchanged mtime means the pre-existing file
          // was never touched.
          if (isDeepStrictEqual(preStat, postStat)) {
            reject(
              new Error(
                `Native zip exited with code 0 but did not modify the archive at '${destination}' (a stale archive from a previous build was left untouched).`
              )
            );
          } else {
            resolve();
...
const nodeZip = async (options) => {
  ...
  const followSymLinks = options.followSymLinks === true;
  // Only track symlinks for the default-behavior warning, which is skipped
  // whenever warnings are disabled (quiet, or warn: false).
  const trackSymLinks =
    typeof options.followSymLinks === "undefined" && warningsEnabled(options);
  const symlinks = [];
