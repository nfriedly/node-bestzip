...
const nodeZip = async (options) => {
  ...
  const storeLinks = options.followSymLinks === false;
  const symlinks = [];
  // Use fs.createWriteStream here (rather than an fsp.open handle) so the
  // stream owns its file descriptor lifecycle. An explicit fsp.open handle
  // left the destination file open/in-flight after nodeZip rejected, which on
  // Windows/macOS made the next test's rmdir of the enclosing temp dir fail
  // with EBUSY/ENOTEMPTY (seen in the command-injection suite).
