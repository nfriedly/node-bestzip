...
function getNativeZipCommand(options) {
  const explicit = (options && options.zipPath) || process.env.BESTZIP_ZIP_PATH;
  if (explicit) {
  ...
  const names = zipExecutableNames();
  for (const dir of DEFAULT_ZIP_DIRS) {
    const found = findInDir(dir, names);
    if (found) {
      return found;
    }
  }
  ...
  // point at the option that opts back into it.
  maybeWarnAboutRefusedZip();
  return null;
}

// PATH entries that bestzip will never treat as a legitimate home for the
// system zip command: empty or relative entries (resolved against the process
// cwd, which is the archive directory under the documented
// `cd build/ && bestzip ../build.zip *` recipe) and anything inside a
// node_modules tree (dependency-provided shims rather than system tooling).
function isUntrustedPathEntry(entry) {
  return (
    !entry ||
    !path.isAbsolute(entry) ||
    entry.split(path.sep).some((part) => part.toLowerCase() === "node_modules")
  );
}
...
function findZipCommandOnUntrustedPath(pathEnv = process.env.PATH) {
  ...
  for (const entry of (pathEnv || "").split(path.delimiter)) {
    if (isUntrustedPathEntry(entry)) {
      continue;
    }
    const found = findInDir(entry, names);
    if (found) {
      return found;
    }
  }
...
// --zip-path on the command line, BESTZIP_ZIP_PATH in the environment) and
// where to request a new default-trusted location. Returns the refused path, or
// null when there was nothing to warn about. A given path is warned at most
// once per process so repeated resolution probes don't spam the build log.
function maybeWarnAboutRefusedZip(pathEnv = process.env.PATH) {
  ...
  console.warn(
    `Warning: bestzip found a \`zip\` command at '${refused}' but it is not ` +
      ...
      `To use this zip, pass --zip-path '${refused}' on the command line, set ` +
      `zipPath: '${refused}' in code, or set BESTZIP_ZIP_PATH='${refused}' ` +
      "in the environment.\n" +
      "If this is a system directory that should be trusted by default, please " +
      "open a ticket or pull request at https://github.com/nfriedly/node-bestzip."
  );
  return refused;
}

function zipExecutableNames() {
  return process.platform === "win32" ? ["zip.exe", "zip"] : ["zip"];
}
...
function findInExplicitLocation(location) {
  ...
  try {
    if (fs.statSync(resolved).isDirectory()) {
      return findInDir(resolved, zipExecutableNames());
    }
  } catch (e) {
